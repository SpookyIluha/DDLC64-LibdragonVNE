...
void engine_eeprom_init(){
    ...
    if(eeprom_type == EEPROM_16K){
        const eepfs_entry_t eeprom_16k_files[] = {
            ...
            { "/autosave.sv",       size_t(sizeof(gamestatus.state))  },
        ...
        result = eepfs_init(eeprom_16k_files, 3);
    }
...
void engine_eeprom_save_autosave(){
    gamestatus.state_persistent.autosaved = true;
    gamestatus.state_persistent.lastsavetype = SAVE_AUTOSAVE;
    engine_eeprom_save_persistent();
    debugf( "Writing '%s'\n", "/autosave.sv" );
    const int result = eepfs_write("/autosave.sv", &gamestatus.state, (size_t)(sizeof(gamestatus.state)));
...
void engine_eeprom_load_autosave(){
    debugf( "Reading '%s'\n", "/autosave.sv" );
    const int result = eepfs_read("/autosave.sv", &gamestatus.state, (size_t)(sizeof(gamestatus.state)));
    if(result != EEPFS_ESUCCESS || gamestatus.state.magicnumber != STATE_MAGIC_NUMBER) 
        {
